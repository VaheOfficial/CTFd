...
async def main():
    ...
    # Open DB session and ensure an author user exists
    db = SessionLocal()
    try:
        author = db.query(User).filter(User.username == 'cli-author').first()
        if not author:
            author = User(
                username='cli-author',
                email='cli-author@example.local',
                role=UserRole.AUTHOR,
                password_hash=get_password_hash(os.getenv('CLI_AUTHOR_PASSWORD', 'change-me-cli'))
            )
            db.add(author)
            db.commit()
            db.refresh(author)
        logger.info(f"Using author user: {author.username} ({author.id})")
        
        # Build a sample request
        request = GenerateChallengeRequest(
            prompt="Generate an easy forensic challenge. The challenge should involve a hosted html page, hide the flag inside the page comments.",
            preferred_provider=LLMProvider.AUTO,
            track=ChallengeTrack.DETECT_FORENSICS,
            difficulty=ChallengeDifficulty.EASY
        )

        # Run generation via service (handles DB + materialization + S3)
        logger.info("Starting challenge generation via AIGenerationService")
        service = AIGenerationService(db)
        response = await service.generate_challenge(request, author)
